...
export class ChainIndex {
  ...
  /**
   * Synchronously read the timestamps and difficulty bits of up to eleven blocks ending at the
   * indexed tip, for header validation (median time past and DigiShield retargeting).
   * @returns {{height: number, recent: Array<{time: number, bits: number|null}>}} Tip height and
   *   oldest-first entries; recent is empty when the rows are unavailable or not contiguous.
   */
  chainContext() {
    let db;
    try {
      db = new Database(path.join(this.dir, 'chainindex.db'), { fileMustExist: false });
      const tip = db.prepare('SELECT MAX(height) AS height FROM blocks').get()?.height ?? -1;
      if (tip < 0) return { height: -1, recent: [] };
      const hasBits = db.prepare('PRAGMA table_info(headers)').all().some(c => c.name === 'bits');
      const rows = db.prepare(`SELECT height, time, ${hasBits ? 'bits' : 'NULL AS bits'} FROM headers WHERE height <= ? ORDER BY height DESC LIMIT 11`).all(tip).reverse();
      const contiguous = rows.length && rows.at(-1).height === tip && rows.every((row, i) => i === 0 || row.height === rows[i - 1].height + 1);
      return { height: tip, recent: contiguous ? rows.map(row => ({ height: row.height, time: row.time, bits: row.bits ?? null })) : [] };
    } catch {
      return { height: -1, recent: [] };
    } finally {
      try { db?.close(); } catch {}
    }
  }
