...
import { parseAuxPow } from './pepenet-wire.js';
...
// Core never sends more than 2,000 headers per message.
const MAX_HEADERS = 2000;
// Bounded peer-discovery state; addr gossip cannot grow memory or crowd out seeds.
const MAX_CANDIDATES = 2000;
const MAX_ADDR_PER_MESSAGE = 1000;
const MAX_INVENTORY = 2000000;

// Distinct peers that must serve a relayed transaction back before it is reported as relayed.
const MIN_RELAY_WITNESSES = 2;

/**
 * Median of the heights advertised by a set of peers (upper median for even counts).
 * @param {Iterable<{remoteStartHeight?: number|null}>} peers - Handshaken peers.
 * @returns {number|null} Median claimed height, or null when no peer advertised one.
 */
function medianHeight(peers) {
  const heights = [...peers].map(peer => peer.remoteStartHeight).filter(Number.isSafeInteger).sort((a, b) => a - b);
...
export class MessageDecoder {
   ...
   */
  push(chunk) {
    // Defer concatenation until a whole frame can be decoded; concatenating on every TCP
    // chunk made a single 32 MiB frame cost quadratic copying.
    this.parts.push(chunk);
    this.partBytes += chunk.length;
    if (this.buffer.length + this.partBytes < this.needed) return [];
    this.buffer = Buffer.concat([this.buffer, ...this.parts]);
    this.parts = [];
    this.partBytes = 0;
    this.needed = 24;
    const messages = [];
    while (this.buffer.length >= 24) {
      if (!this.buffer.subarray(0, 4).equals(PEP_NETWORK.magic)) {
        // A desynchronized stream can never recover; drop it instead of buffering forever.
        this.buffer = Buffer.alloc(0);
        throw new Error('Pepecoin peer sent a frame with the wrong network magic.');
      }
      ...
      if (this.buffer.length < 24 + length) { this.needed = 24 + length; break; }
...
/**
 * Decode base headers and their attached AuxPoW records without validating the proofs.
 * Linkage and proof-of-work checks belong to pepenet-pow.js validateHeaderChain().
 * @param {Buffer} payload - headers message payload.
 * @returns {Array<{header: Buffer, hash: Buffer, previousHash: Buffer, auxpow: import('./pepenet-wire.js').AuxPow|null, time: number, bits: number}>} Header metadata with wire-order hashes.
...
export function parseHeaders(payload) {
  ...
  for (let i = 0; i < count.value; i++) {
    ...
    offset += 80;
    let auxpow = null;
    if (header.readUInt32LE(0) & 0x100) ({ auxpow, offset } = parseAuxPow(payload, offset));
    ...
    await new Promise((resolve, reject) => {
      ...
      socket.once('connect', () => {
        ...
        socket.on('error', error => {
          ...
          // An unhandled 'error' event would crash the host process.
...
export async function broadcastDiscoveredTransaction(
  ...
  await Promise.all(Array.from({ length: Math.min(maxPeers, candidates.length) }, async () => {
    while (next < candidates.length && sent < MIN_RELAY_WITNESSES) {
  ...
  if (sent < MIN_RELAY_WITNESSES) throw new Error(`Transaction relay could not be verified by ${MIN_RELAY_WITNESSES} independent peers (${sent} verified). ${errors.join(' | ')}`);
  return sent;
}
...
export class MultiPeerManager extends EventEmitter {
  ...
  #wirePeer(peer) {
    ...
    peer.on('message', message => {
      if (message.command === 'addr') {
        try {
          for (const address of parseAddresses(message.payload).slice(0, MAX_ADDR_PER_MESSAGE))
  ...
  /**
   * Deduplicate host/port pairs in a bounded first-in, first-out queue.
   * Configured archive peers and DNS seeds are queued first; addr gossip is appended
   * after them so a single peer cannot push its own addresses ahead of the seeds.
   * @private
   * @param {string} host - Candidate host.
   * @param {number} port - Candidate TCP port.
   * @param {bigint} services - Advertised service flags (a peer claim, not trusted).
   * @returns {void}
   */
  #queueCandidate(host, port, services) {
    const key = `${host}:${port}`;
    if (this.candidateKeys.has(key) || this.candidateKeys.size >= MAX_CANDIDATES) return;
    this.candidateKeys.add(key);
    this.candidates.push({ host, port, services });
  }

  /**
   * Choose a full-service peer at random among those claiming at least the median height.
   * Always picking the highest claim let one dishonest peer win every selection.
   * @private
   * @returns {PepecoinPeer|undefined} Selected peer.
   */
  #chooseSyncPeer() {
    if (this.syncPeer && this.readyPeers.has(this.syncPeer) && (this.syncPeer.remoteServices & 1n) === 1n) return this.syncPeer;
    const archive = [...this.readyPeers].filter(candidate => (candidate.remoteServices & 1n) === 1n);
    if (!archive.length) return undefined;
    const median = medianHeight(archive) ?? 0;
    const eligible = archive.filter(candidate => (candidate.remoteStartHeight || 0) >= median);
    return eligible[randomInt(eligible.length)];
  }

  /**
   * Ask independent ready peers whether a block hash is on their best chain.
   * A peer agrees when its getheaders reply is empty or continues directly from the hash.
   * Peers sharing an IPv4 /16 with the sync peer or each other count once.
   * @param {Buffer} tipHash - Wire-order hash of the locally indexed tip.
   * @param {{minPeers?: number, timeoutMs?: number}} [options] - Required agreements (default 2) and deadline.
   * @returns {Promise<{agreed: number, asked: number}>} Agreement counts.
   * @throws {Error} If fewer than minPeers independent peers confirm the tip.
   */
  async confirmTip(tipHash, { minPeers = 2, timeoutMs = 15000 } = {}) {
    const group = peer => (/^\d+\.\d+\./.exec(peer.host || '') || [peer.host])[0];
    const seen = new Set(this.syncPeer ? [group(this.syncPeer)] : []);
    const witnesses = [];
    for (const peer of this.readyPeers) {
      if (peer === this.syncPeer || seen.has(group(peer))) continue;
      seen.add(group(peer)); witnesses.push(peer);
    }
    const ask = peer => new Promise(resolve => {
      const done = result => { clearTimeout(timer); peer.off('message', onMessage); peer.off('close', onClose); resolve(result); };
      const onClose = () => done(false);
      const onMessage = message => {
        if (message.command !== 'headers') return;
        try {
          const count = readVarInt(message.payload);
          done(count.value === 0 || (count.offset + 36 <= message.payload.length && message.payload.subarray(count.offset + 4, count.offset + 36).equals(tipHash)));
        } catch { done(false); }
      };
      ...
      peer.requestHeaders([tipHash]);
    });
    const results = await Promise.all(witnesses.map(ask));
    const agreed = results.filter(Boolean).length;
    if (agreed < minPeers) throw new Error(`Only ${agreed} of ${witnesses.length} independent peers confirmed the indexed chain tip; at least ${minPeers} are required.`);
    return { agreed, asked: witnesses.length };
  }
   ...
   */
  requestHeaders(locatorHashes, stopHash = Buffer.alloc(32)) {
    const peer = this.#chooseSyncPeer();
    if (!peer) return;
    this.syncPeer = peer;
  ...
  async broadcastTransaction(rawTransaction) {
    ...
    // One peer echoing bytes back proves nothing: a dishonest peer can echo invalid data.
    if (sent < MIN_RELAY_WITNESSES) {
      const errors = results
        .filter(result => result.status === 'rejected')
        .map(result => result.reason?.message || String(result.reason));
      throw new Error(`Only ${sent} Pepecoin peer(s) verified the transaction; ${MIN_RELAY_WITNESSES} are required. ${errors.join(' | ')}`);
    }
    return sent;
  }
  ...
  /**
   * Read the median height advertised by ready peers.
   * A single peer's exaggerated claim cannot hold the wallet in a permanent catch-up state.
   * @returns {number|null} Claimed network height, or null if unknown.
   */
  get remoteStartHeight() {
    return medianHeight(this.readyPeers);
  }
