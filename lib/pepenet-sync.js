 ...
 * Every header page is checked for linkage, proof of work (including AuxPoW), DigiShield difficulty,
 * checkpoints and timestamps before bodies are requested; each body must match its header's Merkle root.
 * Script validation and fork rollback are not implemented.
 */
// pepecoin-sync-fast.js
import { EventEmitter } from 'node:events';
import { hash256d, parseBlock, verifyBlockMerkle } from './pepenet-wire.js';
import { parseHeaders } from './pepenet-p2p.js';
import { PEP_CONSENSUS, validateHeaderChain } from './pepenet-pow.js';

/**
 * Mainnet header and block-body checks applied by default.
 * @type {{validateHeaders: function(Array, {height: number, recent: Array}): {height: number, recent: Array}, checkBlock: function(Buffer): void}}
 */
export const MAINNET_CONSENSUS = Object.freeze({
  validateHeaders: (headers, context) => validateHeaderChain(headers, context),
  checkBlock: raw => { verifyBlockMerkle(parseBlock(raw)); }
});
...
export class PepecoinSync extends EventEmitter {
   ...
   * @param {typeof MAINNET_CONSENSUS} [options.consensus] - Header/body validation rules; mainnet by default.
   * @throws {Error} If required transport or index methods are missing.
   */
  constructor({ peer, index, consensus = MAINNET_CONSENSUS }) {
    super();
    ...
    // Timestamps/bits of the most recent blocks, for median-time-past and difficulty checks.
    this.headerContext = this.#initialContext(checkpoint.height);
  ...
  /**
   * Build the validation context for the committed tip.
   * @private
   * @param {number} height - Committed checkpoint height, or -1 for an empty index.
   * @returns {{height: number, recent: Array<{time: number, bits: number|null}>}} Validation context.
   */
  #initialContext(height) {
    const genesis = { time: PEP_CONSENSUS.genesis.time, bits: PEP_CONSENSUS.genesis.bits };
    if (height < 0) return { height: 0, recent: [genesis] };
    let context = { height, recent: [] };
    try { if (typeof this.index.chainContext === 'function') context = this.index.chainContext(); } catch {}
    if (context.height !== height) return { height, recent: [] };
    const recent = context.recent.map(({ time, bits }) => ({ time, bits }));
    if (recent.length && recent.length < PEP_CONSENSUS.medianTimeSpan && context.recent[0].height === 1) recent.unshift(genesis);
  ...
  #onHeaders(payload) {
    try {
      ...
      // Reject the whole page before any body is requested: fabricated headers never reach the index.
      this.headerContext = this.consensus.validateHeaders(headers, this.headerContext);
      previous = this.tipHash;
  ...
  #onBlock(raw) {
    ...
    // Bound memory before retaining anything, and hold only bodies that match their header.
    if (this.pendingBytes + raw.length > MAX_PENDING_BLOCK_BYTES) {
      this.stop();
      this.emit('error', new Error('Peer sent more block data than the pending-block limit allows.'));
      return;
    }
    try { this.consensus.checkBlock(raw); } catch { return; }
