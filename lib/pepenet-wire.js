 ...
 * AuxPoW records are skipped structurally, not consensus-validated. Hash Buffers are wire order.
 *
...
/**
 * Advance past a structurally parsed transaction, discarding its fields.
 * @private
 * @param {Cursor} cursor - Cursor positioned at the transaction start.
 * @returns {void}
 */
function skipTransaction(cursor) { parseTx(cursor); }
/**
 * Advance past parent coinbase, hash branches and parent header.
 * @private
 ...
 */
function skipAuxPowData(cursor) {
  skipTransaction(cursor); cursor.take(32);
  const coinbaseBranch = cursor.varInt(); if (coinbaseBranch > 64) throw new Error('Invalid AuxPoW branch.'); cursor.take(coinbaseBranch * 32); cursor.u32();
  const chainBranch = cursor.varInt(); if (chainBranch > 64) throw new Error('Invalid AuxPoW branch.'); cursor.take(chainBranch * 32); cursor.u32(); cursor.take(80);
}
 ...
 * @returns {number} First byte offset after the record; no proof-of-work verification is performed.
 */
export function skipAuxPow(payload, offset = 0) {
  const cursor = new Cursor(Buffer.from(payload));
  cursor.offset = offset;
  skipAuxPowData(cursor);
...
export function parseBlock(rawBlock) {
  ...
  if (version & 0x100) skipAuxPowData(cursor);
  ...
  return { hash: hash256(header), header, version, time: header.readUInt32LE(68), transactions, blockBytes: transactions.reduce((total, transaction) => total + BigInt(transaction.raw.length), 0n), coinbaseTotal: transactions[0].outputs.reduce((total, output) => total + output.value, 0n) };
}
