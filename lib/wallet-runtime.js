...
export async function openWalletRuntime({ dataDir, autoSync = true, rescan = false, extraIdentities = [] } = {}) {
  ...
  const syncStatus = { state: 'stopped', height: chainIndex.syncCheckpoint().height, startHeight: -1, blocksSynced: 0, targetHeight: null, progressPercent: null, peer: null, error: null, updatedAt: null, lastSyncedAt: null, verifiedHeight: null };
    ...
    try {
      ...
      manager.once('archiveReady', () => {
        ...
        sync.on('synced', block => {
          ...
          // The sync peer alone cannot vouch for its own chain: independent peers must
          // confirm the indexed tip before deposits become eligible or spending resumes.
          Promise.resolve(typeof manager.confirmTip === 'function' ? manager.confirmTip(block.hash) : null).then(() => {
            if (closing || finished) return;
            syncStatus.height = block.height;
            syncStatus.verifiedHeight = block.height;
            ...
            finish('waiting', `Indexed tip not yet confirmed by independent peers: ${error.message}`, 15000);
          });
        });
