...
import { createWallet, encryptPrivateKey, decryptPrivateKey, passwordKey, importWif, wif, destinationScript } from './wallet.js';

/**
 * Read an optional array section of a decrypted backup.
 * @param {unknown} value - Section value.
 * @returns {Array} The array, or an empty array when absent.
 * @throws {Error} If the section is present but not a bounded array.
 */
const list = value => {
  if (value == null) return [];
  if (!Array.isArray(value) || value.length > 100000) throw new Error('Invalid backup.');
  return value;
};
...
export class WalletVault {
  ...
  restore(owner, backup, passphrase) {
    ...
    return this.db.transaction(() => {
      ...
      // Backup contents are caller-supplied: anyone can encrypt a crafted payload with their own
      // passphrase. Only restore rows that belong to wallets in this backup, so a restore cannot
      // reserve or lock another namespace's coins.
      const outpoint = /^[0-9a-f]{64}:\d{1,10}$/;
      const restoredInputs = new Map();
      for (const r of list(data.outgoing)) {
        const walletId = mapped.get(r?.accountId);
        if (!walletId) throw new Error('Invalid backup withdrawal.');
        const identities = new Set(this.addresses(walletId).map(a => a.identity));
        const inputs = JSON.parse(r.details).inputs;
        if (!Array.isArray(inputs) || inputs.some(i => !identities.has(i?.identity))) throw new Error('Invalid backup withdrawal inputs.');
        restoredInputs.set(r.id, new Set(inputs.map(i => `${i.txid}:${i.vout}`)));
        this.db.prepare('INSERT INTO outgoing VALUES (?,?,?,?,?,?,?,?,?,?)').run(r.id, walletId, r.requestId, r.fingerprint, r.txid, r.raw, r.details, r.state, r.error, r.createdAt);
      }
      for (const r of list(data.reservations)) {
        if (typeof r?.outpoint !== 'string' || !outpoint.test(r.outpoint) || !restoredInputs.get(r.outgoingId)?.has(r.outpoint)) throw new Error('Invalid backup reservation.');
        this.db.prepare('INSERT INTO reservations VALUES (?,?)').run(r.outpoint, r.outgoingId);
      }
      for (const r of list(data.coinLocks)) {
        if (!mapped.has(r?.accountId) || typeof r.outpoint !== 'string' || !outpoint.test(r.outpoint)) throw new Error('Invalid backup coin lock.');
        this.db.prepare('INSERT INTO coin_locks VALUES (?,?)').run(mapped.get(r.accountId), r.outpoint);
      }
      for (const c of list(data.contacts)) { destinationScript(c?.address); this.saveContact(owner, c.label, c.address); }
